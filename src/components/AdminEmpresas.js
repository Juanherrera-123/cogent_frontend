...
export default function AdminEmpresas({ credenciales, onAgregar, onEliminar, onEditar }) {
    ...
    const handleGuardarEdicion = async () => {
        ...
        if (!editUsuario.trim() || !editPassword.trim())
            return;
        const ok = await onEditar(credenciales[editIndex].usuario, credenciales[editIndex].empresa || "", editUsuario.trim(), editPassword.trim());
        if (ok) {
            setEditIndex(null);
            setEditUsuario("");
            setEditPassword("");
