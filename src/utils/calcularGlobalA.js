...
export function calcularGlobalAExtrala(puntajeBrutoA, puntajeBrutoExtralaboral) {
    const FACTOR = 616;
...
export function calcularGlobalBExtrala(puntajeBrutoB, puntajeBrutoExtralaboral) {
    ...
    if (global > 100)
        global = 100;
    const baremosGlobal = baremosFormaB.global;
    const baremo = baremosGlobal.find((b) => global >= b.min && global <= b.max);
    return {
        puntajeGlobal: global,
        nivelGlobal: baremo?.nivel ?? "No clasificado"
    };
