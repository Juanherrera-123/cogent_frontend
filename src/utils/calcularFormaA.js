...
const directas = new Set(esquemaFormaA.filter(q => q.esquema === "directo").map(q => q.numero));
const inversas = new Set(esquemaFormaA.filter(q => q.esquema === "inverso").map(q => q.numero));
// Traduce la respuesta a puntaje según su esquema
function respuestaAPuntaje(pregunta, respuesta) {
    const resp = respuesta.toLowerCase().trim();
    if (directas.has(pregunta)) {
        if (resp === "siempre")
            return 0;
        if (resp === "casi siempre")
            return 1;
        if (resp === "algunas veces")
            return 2;
        if (resp === "casi nunca")
            return 3;
        if (resp === "nunca")
...
export function calcularFormaA(respuestas) {
    const mapRespuestas = {};
    ...
    const resultadoDimensiones = {};
    Object.entries(porDimension).forEach(([dimension, preguntas]) => {
        const suma = preguntas.reduce((acc, num) => acc + respuestaAPuntaje(num, mapRespuestas[num] || ""), 0);
        ...
        const nivel = baremo.find((b) => transformado >= b.min && transformado <= b.max)?.nivel ||
            "No clasificado";
        resultadoDimensiones[dimension] = { suma, transformado, nivel };
    });
    const resultadoDominios = {};
    ...
    const sumaTotal = Object.keys(mapRespuestas).reduce((acc, numStr) => acc + respuestaAPuntaje(Number(numStr), mapRespuestas[Number(numStr)] || ""), 0);
